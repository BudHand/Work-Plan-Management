<template>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="p-field">
                            <label for="tot_weight_plan">Weight<span style="color:red;">* </span><span style="font-weight: normal;"> (Current weight: {{ calculateWeight }}/100)</span></label>
                            <InputNumber v-model="formTaskPlan.tot_weight_plan" :min="1" />
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="p-fluid">
                    <div class="form-group">
                                ...
                                <div class="p-field">
                                    ...
                                    <InputNumber v-model="formTaskRealization.weight" />
                                </div>
                            </div>
                </div>
                <template #footer>
                ...
                <div class="card card-outline card-secondary">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                            <Button v-if="!parseInt(formData.launched)" label="New Task" icon="pi pi-plus" class="p-button-secondary p-button-sm" @click="newTaskPlan" />
                            <Button v-if="totalWeightTP == 100 && !parseInt(formData.launched) && user.name === formData.team_leader.name" label="Launch" icon="fas fa-rocket" class="p-button-success p-button-sm" @click="launchTaskPlan" style="margin-left: 5px;" />
        ...
        <div class="row">
            <div class="col-md-12">
                <Toolbar class="p-mb-4">
                    <template #right>
                        <span>
                            ...
                            <Button v-if="today >= firstDay && today <= secondDay" label="Task (Realization)" class="p-button-secondary p-button-sm"  @click="showTaskRealization" style="margin-left: 5px;" />
                        </span>
                    </template>
                </Toolbar>
                <DataTable :value="task" showGridlines :lazy="true" :rows="totalData" ref="dt" :loading="loading" responsiveLayout="scroll" rowGroupMode="rowspan" groupRowsBy="task">
                    ...
                    <Column style="text-align: center;">
                        <template #body="slotProps">
                            {{ slotProps.data.type === 'Realization' ? '' : slotProps.data.tot_weight_plan }}
                        </template>
                    </Column>
                    <Column style="text-align: center;">
                        <template #body="slotProps">
                            {{ slotProps.data.type === 'Plan' ? '' : slotProps.data.tot_weight_realization }}
                        </template>
                    </Column>
                    <Column style="text-align: center;">
                        <template  #body="slotProps">
                            {{ slotProps.data.type === 'Plan' ? '' : slotProps.data.percentage !== null ?  _replaceDotWithComma(slotProps.data.percentage) + '%' : '' }}
                        </template>
                    </Column>
                    <Column style="text-align: center;">
                        <template #body="slotProps">
  ...
  export default {
    ...
    setup() {
        ...
        // Tambahkan logika untuk update status proyek ke Completed
        await updateStatusCompleted(formData);
        // const response = await updateStatusCompleted({id_project : formData.id});
        };
